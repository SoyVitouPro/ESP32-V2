...
// Virtual canvas size (entire chain)
#define VIRTUAL_RES_X (NUM_COLS * PANEL_RES_X)
#define VIRTUAL_RES_Y (NUM_ROWS * PANEL_RES_Y)
...
static int g_panel_rows = NUM_ROWS;
static int g_panel_cols = NUM_COLS;
...
void handleUploadData() {
  ...
  if (up.status == UPLOAD_FILE_START) {
    ...
    if (imgW == 0 || imgH == 0 || imgW > VIRTUAL_RES_X || imgH > VIRTUAL_RES_Y) {
...
void handleUploadDone() {
  ...
  if (!textPixels.empty()) {
    baseY = (int)VIRTUAL_RES_Y / 2 - (int)imgH / 2 + userOffY; // vertical center + offset
    Serial.printf("/upload: drawing at center x~%d y=%d on %dx%d\n", (int)VIRTUAL_RES_X/2, baseY, (int)VIRTUAL_RES_X, (int)VIRTUAL_RES_Y);
    ...
    if (animate) {
      ...
      if (animDir < 0) {
        headX0 = (int)VIRTUAL_RES_X;           // start just off right edge
      ...
      auto blitAt = [&](int xPos){
        ...
        for (int y=0; y<(int)imgH; ++y) {
          int dstY = baseY + y;
          if (dstY < 0 || dstY >= (int)VIRTUAL_RES_Y) continue;
          int srcRow = y * imgW;
          for (int x=0; x<(int)imgW; ++x) {
            int dstX = x0 + x;
            if (dstX < 0 || dstX >= (int)VIRTUAL_RES_X) continue;
            size_t si = (size_t)srcRow + (size_t)x;
            size_t di = (size_t)dstY * (size_t)VIRTUAL_RES_X + (size_t)dstX;
      ...
      blitAt(headX0);
      blitAt(headX1);
      vdisplay->drawRGBBitmap(0, 0, frameBuffer.data(), VIRTUAL_RES_X, VIRTUAL_RES_Y);
      ...
      int16_t x = (int)VIRTUAL_RES_X / 2 - (int)imgW / 2 + userOffX; // horizontal center + offset
      for (int y=0; y<(int)imgH; ++y) {
        int dstY = baseY + y;
        if (dstY < 0 || dstY >= (int)VIRTUAL_RES_Y) continue;
        int srcRow = y * imgW;
...
void handleUploadBgData() {
  ...
  if (up.status == UPLOAD_FILE_START) {
    ...
    bgPixels.assign((size_t)VIRTUAL_RES_X * (size_t)VIRTUAL_RES_Y, bgColor);
    const uint16_t* src = reinterpret_cast<const uint16_t*>(&buf[4]);
    int offx = ((int)VIRTUAL_RES_X - (int)bw) / 2;
    int offy = ((int)VIRTUAL_RES_Y - (int)bh) / 2;
    for (int y=0; y<(int)bh; ++y) {
      int dy = offy + y; if (dy < 0 || dy >= (int)VIRTUAL_RES_Y) continue;
      for (int x=0; x<(int)bw; ++x) {
        int dx = offx + x; if (dx < 0 || dx >= (int)VIRTUAL_RES_X) continue;
        bgPixels[(size_t)dy * VIRTUAL_RES_X + (size_t)dx] = src[(size_t)y * bw + (size_t)x];
      }
    }
    hasBgImage = true;
    // Show background immediately if no text
    if (textPixels.empty()) {
      if (bgPixels.size() == (size_t)VIRTUAL_RES_X * (size_t)VIRTUAL_RES_Y) {
        vdisplay->drawRGBBitmap(0, 0, bgPixels.data(), VIRTUAL_RES_X, VIRTUAL_RES_Y);
      } else {
...
void setup() {
  ...
  // Virtual matrix wrapper for chaining/mapping (use chain macro for serpentine flag)
  vdisplay = new VirtualMatrixPanel(*dma_display, NUM_ROWS, NUM_COLS, PANEL_RES_X, PANEL_RES_Y, VIRTUAL_MATRIX_CHAIN_TYPE);
  // Clear screen
  vdisplay->fillScreen(dma_display->color565(0, 0, 0));
  // Draw seam line at x = PANEL_RES_X to verify two panels linked
  for (int y = 0; y < VIRTUAL_RES_Y; ++y) {
    vdisplay->drawPixel(PANEL_RES_X, y, vdisplay->color565(0, 64, 255));
  }
  frameBuffer.assign((size_t)VIRTUAL_RES_X * (size_t)VIRTUAL_RES_Y, 0);
  ...
  if (animate && !textPixels.empty()) {
    ...
    if (now - lastAnim >= animSpeedMs) {
      ...
      auto blitAt = [&](int xPos){
        ...
        for (int y=0; y<(int)imgH; ++y) {
          ...
          for (int x2=0; x2<(int)imgW; ++x2) {
            ...
            size_t si = (size_t)srcRow + (size_t)x2;
            if (!textAlpha.empty()) {
              if (textAlpha[si]) frameBuffer[(size_t)dstY * VIRTUAL_RES_X + (size_t)dstX] = textPixels[si];
            } else {
              frameBuffer[(size_t)dstY * VIRTUAL_RES_X + (size_t)dstX] = textPixels[si];
            }
          }
        }
      };
      ...
      if (animDir < 0) { // left
        ...
        headX0 += 1; headX1 += 1;
        if (headX0 >= (int)VIRTUAL_RES_X) headX0 = headX1 - spacing;
        if (headX1 >= (int)VIRTUAL_RES_X) headX1 = headX0 - spacing;
      }
    }
  } else {
